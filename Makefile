BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Auf_verbotenen_Wegen_in_Tibet"
TITLE_NAME = "Auf verbotenen Wegen in Tibet"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Auf_verbotenen_Wegen_in_Tibet_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Auf_verbotenen_Wegen_in_Tibet_*.jpg
	rm cover1.jpg 
	rm logo.jpg
